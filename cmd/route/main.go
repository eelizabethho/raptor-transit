...
func main() {
	...
	from := flag.String("from", "", "origin stop_id")
	to := flag.String("to", "", "destination stop_id")
	...
	journeys, err := eng.Query(*from, *to, int32(dep), *date)
	if err != nil {
	...
	if len(journeys) == 0 {
		fmt.Printf("No journey found from %s to %s departing %s on %s.\n",
			stopLabel(tt, *from), stopLabel(tt, *to), *at, *date)
		return
	}
